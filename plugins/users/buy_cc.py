...
[
    ...
    await m.edit_message_text(
        f"""💳 <b>Garantia de 100% live de todas as GG's!</b>\n
{get_info_wallet(m.from_user.id)}""",
        reply_markup=kb,
    )


# Pesquisa de GGs via inline.
@Client.on_inline_query(filters.regex(r"^buscar_(?P<type>\w+) (?P<value>.+)"))
async def search_cc(c: Client, m: InlineQuery):
    """
    Pesquisa uma GG via inline por tipo e retorna os resultados via inline.
...
async def comprar_ccs(c: Client, m: CallbackQuery):
    ...
    await m.edit_message_text(
        f"""<b>💳 Compra GG Unitária</b>
<i> Qual o nível que você deseja Comprar?</i>

<b>⚠️ Avisos!</b>

<i>• Não faço Rembolso.</i>
<i>• Compre somente se souber Usar.</i>
<i>• Para troca de GG'S die Chame: @SuporteCloverStore</i>

<b>💸 ONDE AS GG'S DA STORE PODEM UPAR:</b>

<i>• ELO MORE 650512 - UPANDO FATURA VIVO!</i>\n
<i>• GOLD 498407 - UPANDO ALIEXPRESS EM OUTRAS GATES!</i>\n
<i>• PREPAID 405482 - UPANDO IFOOD, STRIPE, MERCADO LIVRE, UBER E KIWIFY!</i>\n
<i>• AMEX 374769 - UPANDO IFOOD, XBOX GAME PASS, FATURA VIVO E JOGOS DA XBOX!</i>\n""",
        reply_markup=kb,
    )
            ...
            if live_or_die[0]:  # caso venha cc live
                ...
                kb = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="💳 Compre as melhores GG's",url=f"https://t.me/CloverGGsBot"
                            ),
                        ],
                    ]
                )
