        ...
        if rt == "PAGO":
           ## rt = "PAGO"
            ...
            await send.edit_text(
               f"<b>🎉 Pagamento Efetuado! foi adicionado R${value},00 em sua conta no bot.</b>",
                reply_markup=kb,
            )
            ...
            await c.send_message(
                ADMIN_CHAT,
                f"<a href='💰 {mention} <b>Adicionou R${value},00. de saldo no Bot!</b></a>",
            )
...
async def auto_pay(c: Client, m: Union[CallbackQuery, Message]):
    ...
    if isinstance(m, CallbackQuery):
        ...
        cob = await m.message.ask(
            f"""<b>💸 Digite o valor que queira adicionar saldo no Bot! Valor mínimo: R$10,00.

🎈 Problemas ao fazer o Pix? Chame: @AstaCarder</b>""",
...
🎈 Problemas ao fazer o Pix? Chame: @AstaCarder</b>""",
    ...
    base = f"""<b>✅ Pix gerado com Sucesso!</b>

<b>💸 Valor do Pagamento:</b> <i>R${float(value)}0.</i>
<b>⏳ Prazo de Pagamento:</b> <i>5 Minutos.</i>

<b>📃 Pix Copiar e Colar:</b>
<code>{qr}</code>

<b>💡 Dica: Clique em cima do código acima e cole ele na área de tranferência do seu Banco!</b>
"""
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🔕 Já efetuei o Pagamento", callback_data="start"
                )
            ]
        ]
    )
